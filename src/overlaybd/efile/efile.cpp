...
public:
    EFileBuilder(photon::fs::IFile *file, const CryptArgs *args, bool ownership)
...
int is_efile(photon::fs::IFile *file) {
    char buf[EncryptionFile::HeaderTrailer::SPACE];
    auto ret = file->pread(buf, EncryptionFile::HeaderTrailer::SPACE, 0);
    if (ret < (ssize_t)EncryptionFile::HeaderTrailer::SPACE)
...
IFile *new_efile_builder(photon::fs::IFile *file, const CryptArgs *args, bool ownership) {
