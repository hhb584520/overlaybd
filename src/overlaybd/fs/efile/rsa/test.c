...
int main(int argc, char **argv) {
    ...
    //      you may need to do some casting or set the right -W compiler flags to ignore those
    //      warnings (e.g.: -Wno-pointer-sign).
    ...
    // The compression function needs to know how many bytes exist.  Since we're using a string, we
    // can use strlen() + 1 (for \0).
    const int src_size = (int)(strlen(src) + 1);
    // LZ4 provides a function that will tell you the maximum size of compressed output based on
    // input data via LZ4_compressBound().
    ...
    if (compressed_data_size < 0)
        run_screaming(
            "A negative result from LZ4_compress_default indicates a failure trying to compress the data.  See exit code (echo $?) for value returned.",
            compressed_data_size);
    if (compressed_data_size == 0)
        run_screaming(
            "A result of 0 means compression worked, but was stopped because the destination buffer couldn't hold all the information.",
            1);
    if (compressed_data_size > 0)
        printf("We successfully compressed some data!\n");
    // Not only does a positive return_value mean success, the value returned == the number of bytes
    // required. You can use this to realloc() *compress_data to free up memory, if desired.  We'll
    // do so just to demonstrate the concept.
    ...
    /* Decompression */
    // Now that we've successfully compressed the information from *src to *compressed_data, let's
    // do the opposite!  We'll create a *new_src location of size src_size since we know that value.
    char *const regen_buffer = malloc(src_size);
    if (regen_buffer == NULL)
        run_screaming("Failed to allocate memory for *regen_buffer.", 1);
    // The LZ4_decompress_safe function needs to know where the compressed data is, how many bytes
    // long it is, where the regen_buffer memory location is, and how large regen_buffer
    // (uncompressed) output will be. Again, save the return_value.
    ...
    if (decompressed_size == 0)
        run_screaming(
            "I'm not sure this function can ever return 0.  Documentation in lz4.h doesn't indicate so.",
            1);
    if (decompressed_size > 0)
    ...
    /* Validation */
    // We should be able to compare our original *src with our *new_src and be byte-for-byte
    // identical.
