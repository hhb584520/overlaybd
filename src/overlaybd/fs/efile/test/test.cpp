...
class EFileTest : public ::testing::Test {
public:
    ...
    void randread(IFile *fsrc, IFile *fzfile) {
        ...
        while (read_times--) {
            ...
            fefile->pread(data1, len * 512, offset * 512);
        ...
        for (int i = 0; i < 5000; i++) {
            ...
            fefile->pread(large_data1, len * 512, offset * 512);
...
TEST_F(EFileTest, verify_rsa) {
    ...
    if (efile_encrypt(fsrc.get(), fdst.get(), &args) != 0) {
        LOG_ERROR("err: `(`)", errno, strerror(errno));
        return;
    }
    fdst->close();
